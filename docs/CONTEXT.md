...
| D25 | 2026-06-23 | Test/release workflow (post-M9.1) | Staging is the prod compose stack run as a second, ephemeral project (`bibseller-staging`) on the same box behind `test.<domain>`; `main` runs staging, a long-lived `production` branch runs prod, and a release is a `--ff-only` of the tested commit. Promote by fast-forwarding `production` to `main` (or a PR `main` -> `production`). | Localhost cannot exercise the `__Host-session` cookie (needs HTTPS), `cf-ipcountry` (D18), or the baked `PUBLIC_ORIGIN` (D24), so the final pre-deploy check must be a Cloudflare-fronted deploy. Reusing one compose file + Caddyfile (isolated by compose project name + `DB_HOST_PORT`) avoids a second config to maintain; ephemeral staging keeps idle cost zero on the single self-host box (D20). Box uses a git worktree per branch. No CD pipeline yet - deploys stay `make` over SSH. Full flow in DEPLOYMENT.md. |
| D26 | 2026-06-23 | Offsite backups + restore drill (post-M9.1) | Nightly `scripts/offsite-backup.sh` (cron): `pg_dump --no-owner \| gzip` plus an append-only `mc mirror` of the MinIO bucket, both to Cloudflare R2 via the pinned `minio/mc` image; failures email `BACKUP_ALERT_EMAIL` through the Brevo smarthost. `scripts/restore-drill.sh` restores the latest R2 dump into a throwaway `postgres:16`, times it, and asserts the goose schema + key tables (run monthly). | A single self-host box (D20) makes an unreplicated, never-restore-tested `pg_dump` the top risk; this gets backups off the box and proves they restore. Reuses the already-pinned `mc` image (no rclone) and reads `.env.prod` via grep, not `source` (D23). The mirror is append-only so a MinIO wipe cannot delete the offsite copy. R2 as offsite: S3-compatible, free tier, already the object-storage migration target (D16/D21); alerts ride the existing Brevo relay so a down stack still notifies. wal-g/PITR deferred until this dump-and-drill floor exists. |
| D27 | 2026-07-07 | Race-day design language (post-M9.1) | Full frontend rebrand (#198/#199): warm paper surfaces, deep ink blocks, start-gun orange as the only action color, poster hard shadows, condensed uppercase display type, bib-tag cards. Emerald demoted to semantic success/allowed only; policy tones unchanged. Reference: docs/DESIGN.md; source of truth: frontend/src/routes/layout.css. | Founder direction: the pre-#198 UI read as "too computer-sciency" for a consumer marketplace. The system derives from the product's own artifact (the race bib) rather than a UI-kit default; strict brand-vs-semantic color separation keeps the four-policy-mode signals (D2 lineage) intact under any restyle. The slate-* token names deliberately keep their historical names with warm stone values so ~200 scoped usages shifted without edits - rename only with a full mechanical sweep. |
| D28 | 2026-07-08 | "The Runner's Journal" design language (post-M9.1) | Replaces the one-day race-day system (D27) after a staging A/B: ivory paper, warm charcoal, bordeaux as the only action color, Fraunces serif display, hairline rules + soft plate shadows. Token names and the brand-vs-semantic color discipline carry over unchanged; docs/DESIGN.md rewritten. | Founder compared both live (prod race-day vs staging journal) and chose the journal. The shared token architecture (D27) made the second full restyle ~1 day: brand-*/font-display/shadow token values swapped, only the shell/home/cards needed hand edits. Race-day remains in git history if the energy is ever wanted back. |
| D29 | 2026-07-08 | Expand/contract migration discipline (post-M9.1) | Deploy order follows what the migration does, judged against the binary currently running in prod. Expand (additive) -> `prod-migrate` then `prod-up`. Contract (destructive) -> `prod-up` then `prod-migrate`, or land the DROP one release after the code stopped referencing it. Mixed -> split the migration, or accept brief downtime (`prod-build && prod-down && prod-migrate && prod-up`). Rule lives in DEPLOYMENT.md "Migration ordering: expand/contract"; `make promote` points at it instead of hardcoding one order. | Migration 0013 (#175) drops columns the pre-#175 prod binary named on every signed-in query; the previously documented fixed order (`prod-migrate && prod-up`) would have 500'd all signed-in traffic while the new image built (#181). The 2026-07-07 deploy already applied the remediation in practice - inverted order, safe because the post-#175 binary runs correctly against the old schema (leftover columns nullable or defaulted). Same rule in reverse on rollback: rolling the binary back without the Down migration reintroduces queries against dropped columns. |
| D30 | 2026-07-10 | Design language: journal stays, three trial elements kept | A third design trial ("Sunday Run", PR #220 - closed unmerged) confirmed the journal (D28); three elements were ported in its styling via #224: the sticky capsule masthead, semantic policy tones on the home mode cards, and the footer wordmark | Founder compared the trial live on staging vs prod. Trial feedback also set a durable style constraint: no AI-default aesthetics (violet ramps, heavy geometric grotesks, gradient heroes, glassmorphism) - surfaces stay warm, human, grounded in the sport. |
| D31 | 2026-07-10 | M6 payment policy: refund fee borne by buyer; 3-day auto-release | A refund returns `item_amount_cents` only - the itemized processing fee is non-refundable (disclosed at checkout). Auto-release to the seller 3 days after `seller_marked_transferred` unless the buyer confirms or disputes first | Resolves issue #3 Decision 4 the strict-zero-cost way: the platform nets €0 even on refunds. Consequences accepted: checkout copy must disclose non-refundability; chargebacks are the exception (Stripe returns the full charge - the fee cannot be withheld); counsel reviews the ToS wording before payments GA (M7 #11). 3 days because bib handovers verify immediately (registration check), so fast seller payouts win. Detail in [PAYMENTS_AND_COMPLIANCE.md](PAYMENTS_AND_COMPLIANCE.md#fee-reality--pass-through). |
| D32 | 2026-07-10 | Payments idempotency & reconciliation architecture (pre-M6) | Three layers, no generic key table: (1) natural client idempotency - the one-live-order-per-listing index + replay-of-done-transition returns 200; (2) deterministic Stripe idempotency keys per order (`pi:/transfer:/refund:{order_id}`, one PaymentIntent per order) with state-commit-first ordering and `metadata.order_id` on every object; (3) webhook insert-first into `stripe_events` (ON CONFLICT DO NOTHING) processed in the same transaction. One reconciler batchjob backstops all crash windows and runs the deadline duties (TTL - cancelling the PaymentIntent at Stripe before flipping state - auto-release, race-date refunds) | The full failure taxonomy (declined cards, the TTL/late-success race, transfer failure at release, chargebacks held/released, webhook outages) and the platform-balance float note live in [PAYMENTS_AND_COMPLIANCE.md -> Idempotency & failure handling](PAYMENTS_AND_COMPLIANCE.md#idempotency--failure-handling-m6-design); diagrams in `docs/diagrams/`. Constrains M6 (#10) implementation. |
