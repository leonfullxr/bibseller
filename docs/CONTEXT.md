...
- **`Cache-Control: public` on catalog responses** assumes anonymous traffic. ⚠️ **M3 checklist item:** once sessions exist, authed responses must not share caches (add `Vary: Cookie` or gate the header on session absence).
